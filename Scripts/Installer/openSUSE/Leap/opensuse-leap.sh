...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		*)
		...
		wget https://mirrors.tuna.tsinghua.edu.cn/lxc-images/images/opensuse/15.3/${archurl}/default -o tml.html 
		ttime=`sed -n '7p' tmp.html | cut -d'>' -f2 | cut -d'<' -f1`
		rm tmp.html
		wget "https://mirrors.tuna.tsinghua.edu.cn/lxc-images/images/opensuse/15.3/${archurl}/default/${ttime}/rootfs.tar.xz" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
