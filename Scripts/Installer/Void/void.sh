...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		*)
		...
		wget https://mirrors.tuna.tsinghua.edu.cn/lxc-images/images/voidlinux/current/arm64/ -o tmp.html
		ttime=`sed -n '7p' tmp.html | cut -d'>' -f2 | cut -d'<' -f1`
		rm tmp.html
		wget "https://mirrors.tuna.tsinghua.edu.cn/lxc-images/images/voidlinux/current/arm64/${ttime}/rootfs.tar.xz" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
...
wget "https://raw.fastgit.org/EXALAB/AnLinux-Resources/master/Scripts/Installer/Void/resolv.conf" -P void-fs/etc
