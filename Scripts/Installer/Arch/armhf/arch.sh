...
if [ "$first" != 1 ];then
	if [ ! -f $tarball ]; then
		...
		*)
		...
		wget "https://opentuna.cn/archlinuxarm/os/ArchLinuxARM-${archurl}-latest.tar.gz" -O $tarball
	fi
	cur=`pwd`
	mkdir -p "$folder"
	cd "$folder"
...
wget "https://raw.fastgit.org/EXALAB/AnLinux-Resources/master/Scripts/Installer/Arch/armhf/resolv.conf" -P arch-fs/root
wget "https://raw.fastgit.org/EXALAB/AnLinux-Resources/master/Scripts/Installer/Arch/armhf/additional.sh" -P arch-fs/root
echo "done"
