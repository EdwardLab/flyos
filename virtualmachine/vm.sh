...
ABOUT_UTQEMU(){
	clear
printf "%s
${YELLOW}关于FlyOSVirtualMachinePro脚本${RES}
...
ARCH_CHECK() {
	...
	case $(cat ${HOME}/.FlyOSVirtualMachinePro_) in
...
EOF
if [ ! -e ./FlyOSVirtualMachinePro.sh ]; then
curl -O http://shell.eacgh.cn/FlyOSVirtualMachinePro.sh 2>/dev/null
fi
cp FlyOSVirtualMachinePro.sh $sys_name/root/FlyOSVirtualMachinePro.sh
sed -i "s/qemu-system-x86-64-headless/qemu-system-x86 xserver-xorg x11-utils/" $sys_name/root/FlyOSVirtualMachinePro.sh
sed -i 's/qemu-system-i386-headless/-y \&\& apt --reinstall install pulseaudio/' $sys_name/root/FlyOSVirtualMachinePro.sh
#echo "bash FlyOSVirtualMachinePro.sh" >>$sys_name/etc/profile
...
SYSTEM_CHECK() {
	uname -a | grep 'Android' -q
	if [ $? == 0 ]; then
		grep -E -q 'bfsu|tsinghua|ustc|tencent|FlyOSVirtualMachineProcheck' ${PREFIX}/etc/apt/sources.list              
		if [ $? != 0 ]; then  
