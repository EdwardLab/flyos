...
async def get_ubuntu(name, arch, command):
    ...
    await get_rootfs(rootfs_url, name, "gz", command)
...
async def get_centos(name, arch, command):
    ...
    await get_rootfs(rootfs_url, name, "xz", command)
...
async def get_rootfs(self, rootfs_url, name, ext, command):
    popup("正在下载rootfs……")
    await get_result(
        "wget {} -O {}/rootfs/{}.tar.{}".format(rootfs_url, path, name, ext)
    )
    popup("正在解压rootfs……")
    ta_path = os.path.abspath("{}/rootfs/{}/".format(path, name))
    os.makedirs(path)
    fs = tarfile.open(
        "{path}/rootfs/{name}.tar.{ext}".format(path=path, name=name, ext=ext)
    )  # 打开压缩文件
    fs.extractall(path=ta_path)  # 解压所有文件至rootfs目录
    # await get_result(
    #     'tar x{type}vf rootfs/{name}.tar.{ext} -C rootfs/{name}'.format(type='J' if ext == 'xz' else 'gz',
    #                                                                     )
    popup("正在清理……")
    await get_result("rm -f {}/rootfs/{}.tar.{}".format(path, name, ext))
    popup("正在创建配置……")
    conf_template = """
    if [ -z $1 ]; then
