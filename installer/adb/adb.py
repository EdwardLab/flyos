...
def runsys_root(shell):
    try:
        result = subprocess.run(['adb', 'shell', 'su', '-c', shell], capture_output=True, text=True)
...
def check_data_rw():
    temp = popen_sys('su -c "mount -o rw,remount /data"')
    try:
...
def check_system_rw():
    temp = popen_sys('su -c "mount -o rw,remount /system"')
    try:
        ...
        temp = popen_sys('su -c "rm -rf /system/app/test"')
        return True
    except subprocess.CalledProcessError:
        return False
...
def check_internet_connection():
    try:
        result = subprocess.run(['adb', 'shell', 'curl', 'dl.google.com'], capture_output=True, text=True)
